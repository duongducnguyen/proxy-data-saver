...
function AppContent() {
  ...
  useEffect(() => {
    const unsub = window.electronAPI?.update?.onAvailable((info) => {
      setUpdateInfo(info);
    });
    return () => unsub?.();
  }, []);
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-neutral-50 dark:bg-neutral-950">
      ...
      {/* Update Banner */}
      {updateInfo && (
        <div className="flex-shrink-0 mx-6 mb-2 bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800 rounded-lg px-4 py-2.5 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <a
              href="https://app.woware.net/items/proxy-data-saver/4"
              target="_blank"
              rel="noreferrer"
            ...
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 10 10" stroke="currentColor" strokeWidth="1.5">
                <path d="M1 1l8 8M9 1L1 9" />
              </svg>
            </button>
          </div>
