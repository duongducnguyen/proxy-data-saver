    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((card, index) => (
        <div key={index} className="bg-neutral-900/50 rounded-lg p-4">
          ...
          <div className={`text-2xl font-semibold tabular-nums ${card.highlight ? 'text-success-text' : 'text-neutral-100'}`}>
            {card.value}
          </div>
          {card.subValue && (
            <div className="text-xs text-neutral-600 mt-1">{card.subValue}</div>
          )}
        </div>
      ))}
    </div>
