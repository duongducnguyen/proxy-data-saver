  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold text-neutral-100">{t('dashboard.title')}</h2>
        <div className="flex items-center gap-3">
          {proxyPorts.length > 0 && (
            <select
              value={selectedProxy ?? 'all'}
              onChange={(e) => changeProxy(e.target.value === 'all' ? undefined : Number(e.target.value))}
              className="bg-neutral-900 text-neutral-400 text-xs px-2 py-1.5 rounded border border-neutral-800 focus:outline-none focus:border-neutral-700"
              disabled={loading}
            >
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-neutral-900 rounded-lg p-6 max-w-sm mx-4 border border-neutral-800">
            <h3 className="text-base font-medium text-neutral-100 mb-2">{t('dashboard.actions.reset')}</h3>
            <p className="text-sm text-neutral-500 mb-5">{t('dashboard.actions.confirmReset')}</p>
      ...
      {loading && !stats ? (
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-neutral-600">{t('dashboard.noData')}</p>
          </div>
        </div>
      ) : (
